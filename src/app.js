...
Office.onReady(function (info) {
  if (info.host === Office.HostType.Outlook) {
    ...
    initUI(); // still show the UI in other hosts
  }
});

// Fallback: if Office.js never signals ready (e.g. page opened directly in a
// browser tab for testing), initialize anyway after a short delay so the form
// is visible instead of a blank page.
setTimeout(function () {
  if (!window.__availabilityInited) {
    try { initUI(); } catch (e) {
      var s = document.getElementById("status");
      if (s) { s.textContent = "Init error: " + e.message; s.className = "status error"; }
    }
  }
}, 1500);
...
function initUI() {
  if (window.__availabilityInited) return;
  window.__availabilityInited = true;
